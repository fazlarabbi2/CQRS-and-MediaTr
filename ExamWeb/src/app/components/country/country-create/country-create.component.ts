...
export class CountryCreateComponent implements OnInit {
  ...
  onClickUpsertCountry(): void {
    ...
    let isCountryCreateFromValidate: boolean =
      this.getValidateCountryCreateFromResult();

    if (isCountryCreateFromValidate) {
      this.spinnerService.show();
      this.countryService.createAsync(this.countryCreateModel).subscribe(
        (result: CountryCreateModel) => {
        ...
        (error: any) => {
          ...
          this.toastrService.error(
            'Country cannot created! Please, try again.',
            'Error'
          );
          return;
        }
  ...
  private getValidateCountryCreateFromResult(): boolean {
    if (
      this.countryCreateModel.name == undefined ||
      this.countryCreateModel.name == null ||
      this.countryCreateModel.name == ''
    ) {
