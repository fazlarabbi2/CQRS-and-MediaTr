...
export class CountryUpdateComponent implements OnInit {
  ...
  private getCountryById(): void {
    ...
    this.countryService.getByIdAsync(this._countryId!).subscribe(
      (result: CountryViewModel) => {
  ...
  onClickCountryUpdate(): void {
    ...
    if (isCountryFormValidate) {
      this.spinnerService.show();
      this.countryService.updateAsync(this.countryUpdateModel).subscribe(
        (result: CountryUpdateModel) => {
          this.spinnerService.hide();
          this.toastrService.success(
            'Country update successfull.',
            'Successfull'
          );
        ...
        (error: any) => {
          ...
          this.toastrService.error(
            'Country cannot updated! Please, try again.',
            'Error'
          );
        }
      );
  ...
  private getCountryFormValidateResult(): boolean {
    if (
      this.countryUpdateModel.name == undefined ||
      this.countryUpdateModel.name == null ||
